...
import type { MeasureDistanceAnnotation } from '@/types';
import { PopoutWindow } from './PopoutWindow';
import { getDistPopoutWindow, openDistPopoutWindow, closeDistPopoutWindow } from '@/lib/popoutManager';

function LinesCalcPanelInternal() {
    const {
        distCalculatorOpen, setDistCalculatorOpen, distCalculatorColCount, setDistCalculatorColCount,
    ...
    if (!distCalculatorOpen || !activeDoc) return null;
    ...
    const results = distanceAnnotations.map(ann => {
    ...
    const updateCalc = (ann: MeasureDistanceAnnotation, index: number, val: string) => {
    ...
    return (
        <PopoutWindow
            title="Längen (∑) – PDF Editor"
            isOpen={distCalculatorOpen && !!activeDoc}
        ...
        >
            ...
            >
                ...
                >
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <button
                            ...
                            onClick={() => {
                                if (confirm("Alle Längenmessungen in diesem Dokument löschen?")) {
                                    distanceAnnotations.forEach(ann => deleteAnnotation(activeDoc.id, ann.page, ann.id));
                                }
                            }}
                            style={{
                        ...
                        <button
                            onPointerDown={(e) => e.stopPropagation()}
                            onClick={() => setDistCalculatorColCount(Math.max(1, distCalculatorColCount - 1))}
                            style={{
                        ...
                        <button
                            onPointerDown={(e) => e.stopPropagation()}
                            onClick={() => setDistCalculatorColCount(distCalculatorColCount + 1)}
                            style={{
                                background: '#3d3e47', border: 'none', borderRadius: '4px', width: '24px', height: '24px',
                                display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#fff'
                            }}
                    ...
                    <tbody>
                        {results.map(({ ann, base, paddedCalcs, finalResult }, rowIndex) => (
                            ...
                            >
                                ...
                                {Array.from({ length: distCalculatorColCount }).map((_, colIndex) => (
            ...
            {/* Footer Info */}
            <div style={{ padding: '8px 16px', background: '#1a1b1e', fontSize: '10px', color: '#5f6368', borderTop: '1px solid #3d3e47' }}>
...
export const LinesCalcPanel = memo(LinesCalcPanelInternal);
